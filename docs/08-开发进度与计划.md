...
**证据**：
1. **数据模型缺失** - `schema.py` 中无情绪曲线相关数据模型
2. **前端组件缺失** - `novel2script/web/js/` 目录下无情绪曲线组件
3. **后端 API 缺失** - 无情绪曲线数据计算逻辑
...
**证据**：
1. **数据模型缺失** - `schema.py` 中 Character 模型无 `emotion_distribution` 字段
2. **前端组件缺失** - `novel2script/web/js/` 目录下无角色情绪分布雷达图组件
3. **后端 API 缺失** - 无角色情绪分布数据计算逻辑
...
| `novel2script/web/js/app.js` | 修改 | 添加角色面板相关状态变量和方法 |
