...
### 11. 编辑器：左右分栏可拖拽调整比例
- **PRD 来源**：PRD 3.1 节
- **现状**：使用固定 `grid-cols-2`，不可拖拽调整
- **实现**：使用 CM6 或 CSS resize / 自定义拖拽分割条
...
### 14. 编辑器：撤销/重做
- CodeMirror 6 内置 undo/redo 插件，直接启用即可

### 15. 编辑器：使用指南交互
- **PRD 来源**：PRD 3.7 节 — 使用指南可折叠/展开
- **现状**：静态展示，不可折叠
- **实现**：新增折叠按钮 + Alpine.js 控制显示/隐藏
...
### 20. Skill：创建/安装（本地路径）
- 用户通过 UI 创建自定义 Skill 模板
- 从本地路径安装 Skill

### 21. Skill：4 个内置 Skill ⚠️ 部分完成

- **PRD 来源**：PRD 5.2 节
- **现状**：✅ `novel2script/skills/builtins/` 目录已存在
- **已完成**：
...
- **实现**：创建剩余内置 Skill 的 SKILL.md + prompt.yaml + main.py
