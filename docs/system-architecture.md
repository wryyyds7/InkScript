...
> 版本：V1 | 日期：2026-06-06 | 状态：设计阶段
> 输入来源：`docs/requirements-spec.md` + `docs/tech-selection-report.md`
> 设计原则：**插件式扩展，核心不动** —— 每个模块都设计扩展点，后续加功能不改老代码
...
│   │   └── <skill_name>/      # 内置 Skill 目录
│   ├── desktop/                 # 桌面窗口模块
│   │   └── window.py           # PyWebView 窗口封装
│   ├── cli.py                   # CLI 入口（Typer）
│   └── web/                     # 前端静态文件
│       ├── index.html
│       ├── css/
...
**取消检查点**（质询修复 #1）：Pipeline 在以下位置调用 `_check_cancel()`：
1. 每个 Step 执行前（`pipeline.py` `run()` 方法中 `for step_name in self._step_order` 循环开头）
2. `text_segmenter.segment()` 每个分段处理前
3. `merger.merge()` 每个分段结果合并前

```python
# pipeline.py run() 方法中的取消检查
...
#### 10.1.1 LLM 输出格式错误自动修复策略（质询修复 #9）
...
*文档生成时间：2026-06-06*
*输入来源：`docs/requirements-spec.md` + `docs/tech-selection-report.md`*
*质询报告：`docs/architecture-grill-report.md`*
